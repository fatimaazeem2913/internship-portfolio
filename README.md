...
| Day 14 | Week 2 review & full-stack chatbot delivery — a complete capstone: 3 educational activities (Brain Buster, Quick Fire, Ask & Explore) on one platform, real SSE streaming, dual-layer session expiry, structured-generation game logic, and 2 real bugs found through live testing (a CORS header bug, and Day 9's temperature-deprecation finding resurfacing) — 18/18 backend tests passing | [`day-14-capstone/`](./day-14-capstone) |
...
- **Day 14** is the Phase 2 capstone — a complete educational chatbot synthesizing everything built across Days 8-13: three activities sharing one platform via a single routing decision, session IDs traced end-to-end through a real CORS bug found and fixed live, structured generation deciding game state while plain Python (not the LLM) judges correctness, and Day 9's temperature-deprecation finding resurfacing in a genuinely new way — fixed by injecting randomness into prompts instead of a non-functional sampling parameter. Verified with 18/18 real backend tests.
